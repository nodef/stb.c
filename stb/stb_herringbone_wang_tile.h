...
static void stbhw__parse_h_rect(struct stbhw__process *p, int xpos, int ypos,
                            int a, int b, int c, int d, int e, int f)
{
...
static void stbhw__parse_v_rect(struct stbhw__process *p, int xpos, int ypos,
                            int a, int b, int c, int d, int e, int f)
{
   int len = p->c->short_side_len;
   stbhw_tile *h = (stbhw_tile *) malloc(sizeof(*h)-1 + 3 * (len*2) * len);
...
STBHW_EXTERN int stbhw_build_tileset_from_image(stbhw_tileset *ts, unsigned char *data, int stride, int w, int h)
{
   ...
   p.data = data;
   p.stride = stride;
   p.process_h_rect = (stbhw__process_rect *) stbhw__parse_h_rect;
   p.process_v_rect = (stbhw__process_rect *) stbhw__parse_v_rect;
   p.w = w;
   p.h = h;
   p.c = &c;
